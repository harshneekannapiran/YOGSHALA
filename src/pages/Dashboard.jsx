  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const challengeResponse = await fetch(`http://localhost:5000/api/progress/daily-challenge/${userName}`);
        ...
        if (sequenceId) {
          try {
            const response = await fetch(`http://localhost:5000/api/sequences/${sequenceId}`);
            if (response.ok) {
  ...
  const updateProgressInDatabase = async (poseName) => {
    try {
      ...
      if (dailyChallengePose?.name === poseName && completedDailyPoseName !== poseName) {
        const challengeResponse = await fetch('http://localhost:5000/api/progress/daily-challenge/complete', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ username: userName, poseName })
        });
