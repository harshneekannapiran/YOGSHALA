...
const Sequences = ({ userName }) => {
  ...
  useEffect(() => {
    const loadSequences = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:5000/api/sequences/user/${userName}`);
        if (!response.ok) {
        ...
        const defaultResponse = await fetch('http://localhost:5000/api/sequences/defaults');
  ...
  const handleDeleteSequence = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/sequences/${id}`, {
        method: 'DELETE'
      });
